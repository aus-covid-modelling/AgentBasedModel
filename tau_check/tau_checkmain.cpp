...
std::vector<individual> run_model(double beta_C, parameter_struct parameters, std::vector<double> & age_brackets,std::vector<double> & vaccinated_proportion, std::vector<double> & population_pi, std::vector<std::vector<double>> &contact_matrix, vaccine_parameters & no_vaccine, std::vector<vaccine_parameters> & pfizer, std::vector<vaccine_parameters> & astrazeneca, std::vector<vaccine_parameters> & moderna,std::vector<std::vector<double>> & tti_distribution){
    ...
    std::vector<size_t> E_ref; E_ref.reserve(10); // Magic number reserving memory.
    std::vector<size_t> I_ref; I_ref.reserve(10); // Magic number of reserved.
    ...
    int cluster_ref = 0; // Track the exposure number, can show that one dominates.
        while(cluster_ref < 1){
            ...
            double t_in = -20.0;
//            residents[exposed_resident].vaccine_status.vaccinate_individual(pfizer[0], residents[exposed_resident].age_bracket, t_in);
//            residents[exposed_resident].vaccine_status.vaccinate_individual(pfizer[1], residents[exposed_resident].age_bracket, t_in);
            residents[exposed_resident].vaccine_status.vaccinate_individual(astrazeneca[0], residents[exposed_resident].age_bracket, t_in);
            residents[exposed_resident].vaccine_status.vaccinate_individual(astrazeneca[1], residents[exposed_resident].age_bracket, t_in);
...
//    tau_D1_c_symptomatic    0.54
//    tau_D1_a_symptomatic    0.52
//    tau_D2_c_symptomatic    0.35
//    tau_D2_a_symptomatic    0.35
//    tau_D0_symptomatic    1
//    tau_D1_c_asymptomatic    0.27
//    tau_D1_a_asymptomatic    0.26
//    tau_D2_c_asymptomatic    0.175
//    tau_D2_a_asymptomatic    0.175
//    tau_D0_asymptomatic    0.5
        ...
        if(output_file.is_open()){
            ...
            for(individual & person: residents){
                ...
                if(is_infected){
                    ...
                    output_file << person.covid.time_of_recovery << ", " << person.covid.cluster_number << ", " << person.who_infected.size() << ", " << person.total_contacts << std::endl;
                }else{
